...
resource "aws_alb" "nats_server2_route_lb" {
  name               = "nats-server2-route-lb"
...
resource "aws_lb_target_group" "nats_server2_route_target_group" {
  name        = "nats-server2-route-target-group"
  port        = "${var.route_port}"
  protocol    = "TCP"
  target_type = "ip"
...
resource "aws_lb_listener" "nats_server2_route_listener" {
  load_balancer_arn = "${aws_alb.nats_server2_route_lb.arn}" 
  port              = "${var.route_port}"
  protocol          = "TCP"
  default_action {
    type             = "forward"
